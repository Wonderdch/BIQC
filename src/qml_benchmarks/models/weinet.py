...
class WeiNet(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        self.manual_n_qubits = manual_n_qubits
        self.jit = jit
    ...
    def initialize(self, n_features, classes=None):
        ...
        if self.manual_n_qubits == -1:
            # 使用原来的自动计算逻辑
            self.n_qubits_ = 2 * ceil(jnp.log2(im_height))
        else:
            # 使用手动指定的量子比特数量
            self.n_qubits_ = self.manual_n_qubits
            # 确保量子比特数量是偶数（因为模型架构需要）
            if self.n_qubits_ % 2 != 0:
    ...
    def print_model_info(self):
        ...
        print("模型架构:")
        print("WeiNet 量子卷积神经网络")
        print(f"输入维度: {self.n_qubits_} 量子比特")
        print(f"输出维度: 1 (二分类)")
        ...
        print("\n每层可训练参数数量:")
        for name, param in self.params_.items():
            if isinstance(param, dict):
                for sub_name, sub_param in param.items():
                    print(f"{name}.{sub_name}: {sub_param.size} 参数")
            else:
                print(f"{name}: {param.size} 参数")
