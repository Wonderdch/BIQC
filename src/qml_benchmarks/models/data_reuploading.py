...
class DataReuploadingClassifier(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        r"""
        "Data re-uploading for a universal quantum classifier" from https://arxiv.org/abs/1907.02085.
        Here we code the 'multi-qubit classifier'.
        ...
        def circuit(params, x):
            ...
            for layer in range(self.n_layers):
                ...
                for i in range(self.n_qubits_):
                    # scaled inputs，可训练的缩放参数 omegas
                    # x 中的每 3 个元素分别与对应的 omega 相乘，得到第一个旋转门的三个旋转角度
                    angles = (
                            x[x_idx: x_idx + 3]
                ...
                if layer % 2 == 0:
                    # 偶数层，使用 pattern="double" 模式，将CZ门应用于相邻的量子比特对，例如: (0,1), (2,3), (4,5), 等等
                    qml.broadcast(qml.CZ, range(self.n_qubits_), pattern="double")
                else:
                    # 奇数层，使用 pattern="double_odd" 模式，将CZ门应用于错开的量子比特对，例如: (1,2), (3,4), (5,6), 等等
                    qml.broadcast(qml.CZ, range(self.n_qubits_), pattern="double_odd")

            # final reupload without CZs，最后一层的重上传无需CZ门
            x_idx = 0
            for i in range(self.n_qubits_):
                angles = (
                        x[x_idx: x_idx + 3]
    ...
    def fit(self, X, y):
        ...
        def loss_fn(params, x, y):
            ...
            # 准备 alpha 矩阵，它就是论文中提到的 类权重 class weights，它就是加权保真度的那个 权
            # repeated rows of alpha parameters
            alpha_mat0 = jnp.vstack([params["alphas"][0, :] for __ in range(len(x))])
...
class DataReuploadingClassifierNoCost(DataReuploadingClassifier):
    """
    用于验证原始模型中基于保真度(fidelity)的损失函数是否有效
    修改了损失函数，直接基于分类错误的概率
    """
