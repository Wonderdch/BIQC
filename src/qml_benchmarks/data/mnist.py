...
def generate_mnist(
    ...
    if preprocessing == "keep_high":
        ...
        # 选择指定的两个数字
        train_mask = (mnist_train.targets == digitA) | (mnist_train.targets == digitB)
        test_mask = (mnist_test.targets == digitA) | (mnist_test.targets == digitB)

        X_train = mnist_train.data[train_mask]
        y_train = mnist_train.targets[train_mask]
        X_test = mnist_test.data[test_mask]
        y_test = mnist_test.targets[test_mask]

        # 转换标签为 -1 和 1
        y_train = torch.where(y_train == digitA, -1, 1)
        y_test = torch.where(y_test == digitA, -1, 1)

        # 定义图像预处理流程
        transform = transforms.Compose([
            transforms.ToPILImage(),
            transforms.Resize(height),  # 保持原始大小或稍微增加
