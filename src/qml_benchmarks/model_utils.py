...
def train(
    ...
    for step in range(model.max_steps):
        ...
        if np.isnan(loss_val):
            ...
            model.converge_step_ = step
            break

        # 判断模型是否收敛
        if step > 2 * convergence_interval:
            # get means of last two intervals and standard deviation of last interval
            average1 = np.mean(loss_history[-convergence_interval:])  # 最近一个 interval 的 loss 平均值
            average2 = np.mean(  # 倒数第二个 interval 的 loss 平均值
                loss_history[-2 * convergence_interval: -convergence_interval]
            )
            std1 = np.std(loss_history[-convergence_interval:]) # 最近一个 interval 的 loss 标准差
            # if the difference in averages is small compared to the statistical fluctuations, stop training.
            # 如果两个 interval 的 loss 平均值差异小于最近一个 interval 的 loss 标准差的某个量级，则认为模型收敛
    ...
    if hasattr(model, 'plot_identifier') and model.plot_identifier:
        save_loss_figure(model, model.loss_history_)

    if not converged:
        model.converge_step_ = model.max_steps
        logging.warning(f"Model {model.__class__.__name__} has not converged after {model.max_steps} steps.")

        # 不抛出 Warning，以免 GridSearchCV 将评估指标都设置为 NAN
...
def save_loss_figure(model, loss_history):
    """保存损失曲线
    
    Args:
        model: 模型实例，需要有 plot_identifier 属性和 get_params() 方法
        loss_history: 训练过程中记录的损失值列表
    """
    ...
    # 添加标识信息
    if hasattr(model, 'plot_identifier') and model.plot_identifier:
        title += f"_{model.plot_identifier}"
